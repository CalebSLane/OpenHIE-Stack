...
function import_sources() {
  echo "CLUSTERED_MODE=${CLUSTERED_MODE}"
...
function check_elastic() {
  if [[ ! $(docker::get_current_service_status "elasticsearch_$ES_LEADER_NODE") == *"Running"* ]]; then
    log error "FATAL: Elasticsearch is not running, fluentd is dependant on it\n"
    exit 1
  fi
}

function initialize_package() {
  local fluentd_dev_compose_filename=""
  if [[ "${MODE}" == "dev" ]]; then
    log info "Running package in DEV mode"
    fluentd_dev_compose_filename="docker-compose.dev.yml"
  else
  ...
  (
    ...
    docker::deploy_service $STACK "${COMPOSE_FILE_PATH}" "docker-compose.yml" "$fluentd_dev_compose_filename"
  ) || {
